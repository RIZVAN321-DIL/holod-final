...
# ---------- ЗАПИСЬ КЛИЕНТА (ИЗМЕНЕНО: убран выбор услуги, сразу предлагается способ записи) ----------
...
async def client_booking(message: Message, state: FSMContext):
    ...
    if get_active_order_count(user_id) >= 1:
        ...
        return
    await state.clear()
    # Услуга фиксированная - установка
...
# Удалён обработчик service_chosen, так как он больше не нужен. Все остальные обработчики остаются без изменений.
...
# ---------- ОТМЕНИТЬ/ПЕРЕНЕСТИ (полностью рабочий код) ----------
    ...
    if success:
        ...
        if order[1] != 0:
            ...
            await bot.send_message(ADMIN_ID,
                ...
                f"Клиент: {order[5]}\n"
                f"Телефон: {format_phone(order[6])}\n"
        ...
        if success:
            ...
            if user_id != 0:
                ...
                await bot.send_message(ADMIN_ID,
                    ...
                    f"Клиент: {name}, тел: {format_phone(phone)}\n"

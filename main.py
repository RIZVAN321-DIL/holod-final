...
# ---------- ОБРАБОТКА ТЕЛЕФОНА (автоматический или ручной) ----------
...
async def manual_city_input(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(BookingState.city)   # остаёмся в том же состоянии, ждём текст
    await callback.answer()
...
async def admin_cancel_start(message: Message, state: FSMContext):
    if message.from_user.id != ADMIN_ID:
        return
    await message.answer("Введите ID записи для отмены (цифру).")
...
async def admin_confirm_cancel(callback: CallbackQuery, state: FSMContext):
    ...
    if current_state != AdminCancelState.confirm:
        await callback.message.edit_text("❌ Операция не активна. Начните заново.")
        await state.clear()
        return
    data = await state.get_data()

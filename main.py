...
from keyboards import (
    ...
    move_bookings_keyboard, cancel_or_move_keyboard, stats_or_history_keyboard,
    cancel_move_back_keyboard
)
...
# ---------- ПОИСК СВОБОДНОГО СЛОТА ДЛЯ ПЕРЕНОСА ----------
...
async def help_cmd(message: Message):
    ...
    if user_id == ADMIN_ID:
        help_text = (
            ...
            "📞 <b>Запись (звонок)</b> – добавить клиента\n"
            "❌ <b>Отменить/Перенести</b> – отмена заказа по ID или перенос на ближайшее время\n"
...
# ---------- АДМИН: ОТМЕНИТЬ/ПЕРЕНЕСТИ ----------
@dp.message(F.text == "❌ Отменить/Перенести")
async def admin_cancel_move_start(message: Message, state: FSMContext):
    if message.from_user.id != ADMIN_ID:
        return
    await message.answer("Введите ID записи (можно посмотреть в «Все записи») или нажмите «Назад»:", reply_markup=cancel_move_back_keyboard())
    await state.set_state(AdminCancelMoveState.waiting_for_id)

@dp.message(AdminCancelMoveState.waiting_for_id)
async def admin_cancel_move_id(message: Message, state: FSMContext):
    # Если пришло не число – игнорируем (кнопка «Назад» обрабатывается отдельно)
    try:
        order_id = int(message.text.strip())
    except:
        await message.answer("❌ Введите число или нажмите «Назад».", reply_markup=cancel_move_back_keyboard())
        return
    order = get_order_by_id(order_id)
    if not order or order[9] != 'active':
        await message.answer("❌ Заказ не найден или уже отменён.", reply_markup=cancel_move_back_keyboard())
        return
    await state.update_data(order_id=order_id)
    await message.answer(f"Выберите действие для заказа #{order_id}:", reply_markup=cancel_or_move_keyboard())
    await state.set_state(AdminCancelMoveState.choice)

@dp.callback_query(F.data == "admin_cancel_only", StateFilter(AdminCancelMoveState.choice))
...
@dp.callback_query(F.data == "admin_move_booking", StateFilter(AdminCancelMoveState.choice))
async def admin_move_booking(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"Ближайший свободный слот: {format_date(new_date)} {new_slot}\n"
                                     f"Перенести запись #{order_id} на это время?", reply_markup=cancel_or_move_keyboard())
    await state.set_state(AdminCancelMoveState.confirm_move)

@dp.callback_query(F.data == "confirm_yes", StateFilter(AdminCancelMoveState.confirm_move))
async def admin_confirm_move(callback: CallbackQuery, state: FSMContext):
    ...
    if not order or order[9] != 'active':
        ...
        return
    old_date, old_slot = order[3], order[4]
    save_cancelled_order(order_id, order[1], order[5], order[6], order[7], order[8],
                         order[2], old_date, old_slot, "move")
...
@dp.callback_query(F.data == "confirm_no", StateFilter(AdminCancelMoveState.confirm_move))
async def admin_cancel_move_no(callback: CallbackQuery, state: FSMContext):
    await callback.message.edit_text("❌ Операция отменена.")
    await state.clear()
    await callback.message.answer("Меню", reply_markup=admin_menu())
    await callback.answer()

@dp.callback_query(F.data == "cancel_move_back")
async def cancel_move_back(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    await callback.message.edit_text("❌ Операция отменена.")
...
async def move_bookings_confirm(callback: CallbackQuery, state: FSMContext):
        ...
        if old_date:
            ...
            try:
                await bot.send_message(user_id,
                    ...
                    f"Было: {format_date(old_date)} {old_slot_}\n"
                    f"Стало: {format_date(new_date)} {new_slot}\n\n"
    ...
    if success:
        await callback.message.edit_text(f"✅ <b>День {format_date(block_date)} заблокирован.</b>\nПеренесено: {moved}, ошибок: {errors}")
        now = now_moscow()
        year = now.year
        month = now.month
        blocked_dates = get_blocked_dates()

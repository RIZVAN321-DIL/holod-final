...
def get_orders_page(orders, page, per_page=5):
    start = page * per_page
    end = start + per_page
...
async def send_orders_page(message_or_callback, state, page=0):
    ...
    # Преобразуем в кортежи для клавиатуры (с user_id для отображения иконки)
    orders_for_kb = []
...
# ---------- НОВАЯ ЛОГИКА: ОТМЕНИТЬ/ПЕРЕНЕСТИ (СО СПИСКОМ ЗАКАЗОВ) ----------
...
async def admin_cancel_only(callback: CallbackQuery, state: FSMContext):
    ...
    if success:
        ...
        # Отправляем уведомление только если это не телефонная запись (owner_id != 0)
...
async def admin_confirm_move(callback: CallbackQuery, state: FSMContext):
    ...
    if success:
        ...
        # Уведомляем только если это не телефонная запись
        if order[1] != 0:
            try:

...
from keyboards import (
    ...
    stats_or_history_keyboard, orders_list_keyboard
)
...
# ---------- ПОИСК БЛИЖАЙШЕГО СВОБОДНОГО СЛОТА (ИСПРАВЛЕНО) ----------
async def find_next_free_slot(start_date: str = None, avoid_date: str = None):
    """
    Находит ближайший свободный слот начиная с сегодняшнего дня (или с start_date, если указано).
    Возвращает (date_str, slot) или (None, None).
    """
    if start_date is None:
        start_date = now_moscow().strftime("%Y-%m-%d")
    else:
    ...
    for i in range(30):
        ...
        if avoid_date and date_str == avoid_date:
            continue
...
# ---------- ФУНКЦИЯ ПОКАЗА СПИСКА ЗАКАЗОВ (РАБОТАЕТ) ----------
async def send_orders_list(target, state, page=0):
    orders = get_all_future_orders()
    if not orders:
        text = "📭 <b>Нет будущих записей.</b>"
        if isinstance(target, CallbackQuery):
            await target.message.edit_text(text)
            await target.message.answer("Меню", reply_markup=admin_menu())
        else:
            await target.answer(text)
            await target.answer("Меню", reply_markup=admin_menu())
        await state.clear()
        return
    total = len(orders)
    ...
    for o in page_orders:
        ...
        orders_for_kb.append((order_id, user_id, service, date_str, slot, name, phone))
    kb = orders_list_keyboard(orders_for_kb, page, total_pages)
    text = "📋 <b>Выберите запись для отмены или переноса:</b>"
    if isinstance(target, CallbackQuery):
...
async def help_cmd(message: Message):
    ...
    if user_id == ADMIN_ID:
        help_text = (
            ...
            "📋 <b>Все записи</b> – список будущих заказов (📞 – по телефону)\n"
            "📞 <b>Быстрая запись (по звонку)</b> – добавить клиента\n"
            "❌ <b>Отменить/Перенести</b> – выбор записи из списка, затем отмена или перенос\n"
...
async def admin_all_orders(message: Message):
    ...
    for o in orders:
        order_id, user_id, service, date_str, slot, name, phone, city, address = o
        date_display = format_date(date_str)
        phone_icon = "📞 " if user_id == 0 else ""
        text += (
            f"┌ {phone_icon}<b>ID {order_id}</b>\n"
...
async def admin_save_booking(callback: CallbackQuery, state: FSMContext):
    ...
    if not is_slot_free(data['date'], data['slot']):
        ...
        await state.clear()
        return
    # user_id = 0 означает запись по телефону (нет Telegram-пользователя)
...
async def admin_cancel_move_start(message: Message, state: FSMContext):
    ...
    await send_orders_list(message, state, 0)
...
async def orders_page(callback: CallbackQuery, state: FSMContext):
    ...
    if page < 0 or page >= total_pages:
        ...
        return
    await send_orders_list(callback, state, page)
    await callback.answer()
...
async def admin_move_booking(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    order_id = data.get("selected_order_id")
    if not order_id:
        await callback.message.edit_text("❌ Ошибка, заказ не выбран.")
    ...
    if not order or order[9] != 'active':
        ...
        await state.clear()
        return
    # Поиск ближайшего свободного слота начиная с сегодня
    new_date, new_slot = await find_next_free_slot()
    if not new_date:
        await callback.message.edit_text("❌ Нет свободных слотов для переноса в ближайшие 30 дней.")
        await state.clear()
        await callback.message.answer("Меню", reply_markup=admin_menu())
        return
    ...
    await state.set_state(AdminCancelMoveState.confirm_move)

@dp.callback_query(F.data == "confirm_move_yes", StateFilter(AdminCancelMoveState.confirm_move))
async def admin_confirm_move(callback: CallbackQuery, state: FSMContext):
    ...
    if not order or order[9] != 'active':
        ...
        return
    old_date, old_slot = order[3], order[4]
    # Сохраняем в историю как перенос
    ...
    if success:
        await callback.message.edit_text(f"✅ <b>Заказ #{order_id} перенесён на {format_date(new_date)} {new_slot}.</b>")
        if order[1] != 0:
            try:
            ...
            # Телефонная запись – уведомляем админа
            await bot.send_message(ADMIN_ID,
                f"📞 <b>Требуется звонок клиенту по телефону!</b>\n"
                f"Запись №{order_id} (по телефону) перенесена.\n"
                f"Клиент: {order[5]}\n"
                f"Телефон: {order[6]}\n"
                f"Новое время: {format_date(new_date)} {new_slot}\n"
                f"Не забудьте предупредить о переносе."
            )
    else:
        await callback.message.edit_text("❌ Ошибка при переносе.")
    await state.clear()
    await callback.message.answer("Меню", reply_markup=admin_menu())
    await callback.answer()

@dp.callback_query(F.data == "confirm_move_no", StateFilter(AdminCancelMoveState.confirm_move))
...
async def move_bookings_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    for book in bookings:
        ...
        # Поиск ближайшего свободного слота, избегая блокируемого дня
        new_date, new_slot = await find_next_free_slot(avoid_date=block_date)
        if not new_date:
            errors += 1
            continue
        success, old = await move_booking(order_id, new_date, new_slot)
        if success:
            old_date, old_slot_ = old
            ...
            if user_id != 0:
                ...
                # Телефонная запись – уведомляем админа
                await bot.send_message(ADMIN_ID,
                    f"📞 <b>Автоматический перенос (выходной день) - требуется звонок!</b>\n"
                    f"Запись №{order_id} (по телефону) перенесена с {format_date(block_date)} на {format_date(new_date)} {new_slot}.\n"
                    f"Клиент: {name}, тел: {phone}\n"
                    f"Позвоните и предупредите.")
        else:
            errors += 1

...
from keyboards import (
    ...
    stats_or_history_keyboard, cancel_move_back_keyboard
)
...
async def move_bookings_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    for book in bookings:
        ...
        start_search = (now_moscow().date() + timedelta(days=1)).strftime("%Y-%m-%d")
        new_date, new_slot = await find_next_free_slot(start_search, avoid_date=block_date)
        if not new_date:
            errors += 1
            continue
        success, old_info = await move_booking(order_id, new_date, new_slot)
        if success:
            old_date, old_slot_ = old_info
            ...
            try:
                await bot.send_message(
                    ...
                    f"Если новое время вам не подходит или вас не будет дома, пожалуйста, отмените запись через бота (кнопка «Мои записи») и запишитесь заново.\n"

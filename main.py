...
async def move_booking(order_id, new_date, new_slot):
    ...
    if not is_slot_free(new_date, new_slot):
        return False, None
    # Освобождаем старый слот и занимаем новый
...
async def admin_cancel_move_start(message: Message, state: FSMContext):
    if message.from_user.id != ADMIN_ID:
        return
    await message.answer("Введите ID записи (можно посмотреть в «Все записи»).")
...
async def admin_cancel_only(callback: CallbackQuery, state: FSMContext):
    ...
    old_date = order[3]
    old_slot = order[4]
    # Отменяем с сохранением в историю
...
async def admin_move_booking(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"Ближайший свободный слот: {format_date(new_date)} {new_slot}\n"
                                     f"Перенести запись #{order_id} на это время?", reply_markup=confirm_keyboard())
...
async def admin_confirm_move(callback: CallbackQuery, state: FSMContext):
    ...
    if not order or order[9] != 'active':
        ...
        return
    old_date, old_slot = order[3], order[4]
    # Сохраняем в историю факт переноса (как отмену с причиной "move")
...
async def move_bookings_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    for book in bookings:
        ...
        if old_date:
            # Сохраняем в историю факт переноса
...
async def admin_history(callback: CallbackQuery):
    ...
    for c in cancels:
        # c = (id, original_order_id, client_name, phone, city, address, service, original_date, original_slot, cancelled_at, reason)
        text += (

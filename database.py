...
def format_phone(phone: str) -> str:
    """Приводит номер к виду +7XXXXXXXXXX (кликабельно для Telegram)"""
    ...
    if digits.startswith('8'):
        digits = '7' + digits[1:]
    # Если не начинается с 7, добавляем 7 (предполагаем российский номер)
    if not digits.startswith('7'):
        digits = '7' + digits
    # Итоговый формат: +7XXXXXXXXXX (11 цифр)
    if len(digits) >= 11:
        digits = digits[:11]
    return '+' + digits

...
from database import (
    ...
    save_cancelled_order, update_order_slot, format_phone
)
...
# ---------- ЗАПИСЬ КЛИЕНТА (убрали лишний выбор услуги) ----------
...
async def client_booking(message: Message, state: FSMContext):
    ...
    if get_active_order_count(user_id) >= 1:
        ...
        return
    await state.clear()
    await state.update_data(service="Установка кондиционера")
    await message.answer("Как хотите записаться?", reply_markup=quick_or_manual())
    await state.set_state(BookingState.date)
...
async def confirm_booking_stage(msg, state):
    ...
    text = (
        ...
        f"👤 <b>Имя:</b> {data['name']}\n"
        f"📞 <b>Телефон:</b> {format_phone(data['phone'])}\n"
...
async def admin_confirm_move(callback: CallbackQuery, state: FSMContext):
    ...
    if success:
        ...
        if order[1] != 0:
            ...
            # телефонная запись – уведомляем админа с сырым номером (кликабельно)
            raw_phone = order[6]  # исходный номер (в БД может быть с +7 или без)
            # используем format_phone для приведения к +7XXXXXXXXXX, но без пробелов (это уже делает format_phone)
            clickable_phone = format_phone(raw_phone)
            await bot.send_message(ADMIN_ID,
                f"📞 <b>Требуется звонок клиенту по телефону!</b>\n"
                f"Запись №{order_id} (по телефону) перенесена.\n"
                f"Клиент: {order[5]}\n"
                f"Телефон: {clickable_phone}\n"
    ...
    for book in bookings:
        ...
        if success:
            ...
            if user_id != 0:
                ...
                clickable_phone = format_phone(phone)
                await bot.send_message(ADMIN_ID,
                    ...
                    f"Клиент: {name}, тел: {clickable_phone}\n"

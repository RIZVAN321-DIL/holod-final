...
# ---------- ФУНКЦИЯ ПОКАЗА СПИСКА ЗАКАЗОВ (ИСПРАВЛЕНА) ----------
async def send_orders_list(target, state, page=0):
    """
    target: Message или CallbackQuery
    Отправляет или обновляет сообщение со списком заказов.
    """
...
async def move_bookings_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    for book in bookings:
        ...
        if success:
            ...
            moved += 1
            # Уведомляем только если user_id != 0 (не телефонная запись)
            if user_id != 0:
                try:

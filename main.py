...
async def admin_move_booking_choice(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("🔄 <b>Выберите способ переноса:</b>", reply_markup=admin_move_choice_keyboard())
    # остаёмся в том же состоянии choosing_action для обработки выбора
    await callback.answer()
...
async def manual_move_selected(callback: CallbackQuery, state: FSMContext):
    ...
    month = now.month
    blocked_dates = get_blocked_dates()
    await callback.message.edit_text("📅 Выберите НОВУЮ дату для переноса:", 
...
async def admin_new_slot_chosen(callback: CallbackQuery, state: FSMContext):
    ...
    if not new_date or not order_id:
        await callback.message.edit_text("❌ Ошибка, данные потеряны. Начните заново.")
        await state.clear()
        return
    ...
    await callback.message.edit_text(confirm_text, reply_markup=confirm_move_keyboard())
...
async def admin_confirm_move(callback: CallbackQuery, state: FSMContext):
    ...
    if success:
        ...
        await callback.message.edit_text("❌ Ошибка при переносе (возможно, слот занят).")
    await state.clear()
    await callback.message.answer("Меню", reply_markup=admin_menu())
    await callback.answer()

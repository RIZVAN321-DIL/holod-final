...
def main_menu():
    buttons = [
        ...
        [KeyboardButton(text="❄️ Обслуживание"), KeyboardButton(text="📞 Консультация")],
...
def service_buttons():
    # Сохранено для совместимости, но в основном коде не используется (лишний шаг убран)
...
def orders_list_keyboard(orders, page, total_pages):
    ...
    for order in orders:
        order_id, user_id, service, date_str, slot, name, phone = order
        display_date = datetime.strptime(date_str, "%Y-%m-%d").strftime("%d.%m")
        phone_icon = "📞 " if user_id == 0 else ""
        text = f"{phone_icon}ID {order_id}: {display_date} {slot} | {name}"
